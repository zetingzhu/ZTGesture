 ...
 * 移动改变视图大小
 */
public class MoveResizeLayout extends ConstraintLayout {
    ...
    public MoveResizeLayout(@NonNull Context context) {
        super(context);
        init();
    }

    public MoveResizeLayout(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public MoveResizeLayout(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void init() {
        viewDragHelper = ViewDragHelper.create(this, new ViewDragHelper.Callback() {
            ...
            public boolean tryCaptureView(@NonNull View child, int pointerId) {
                boolean gstView = child == gst_view;
            ...
            public int clampViewPositionVertical(@NonNull View child, int top, int dy) {
                return Math.min(gstOpenDefaultHeight, Math.max(gstMinHeight, (int) (top + dy * slipRate)));
            }
        });
...
//        Log.d(TAG, "上下移动：left:" + left + " top:" + top + " dx:" + dx + " dy:" + dy);
        if (gst_top != null) {
            // 上方高度
            ViewGroup.LayoutParams tap = gst_top.getLayoutParams();
            tap.height = top;
//            Log.d(TAG, "上下移动 > 顶部视图高度 top:" + top);
    ...
    public void viewReleased(@NonNull View releasedChild, float xvel, float yvel) {
        int gstTop = releasedChild.getTop();
